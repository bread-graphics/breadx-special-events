...
use breadx::{Result, protocol::Event, display::{DisplayBase, Display, RawReply, RawRequest}};
use slab::Slab;

#[cfg(feature = "async")]
use breadx::display::{AsyncStatus, CanBeAsyncDisplay, AsyncDisplay, AsyncDisplayExt};
#[cfg(feature = "async")]
...
impl<Dpy: DisplayBase + ?Sized> SpecialEventDisplay<Dpy> {
    ...
    fn poll_for_some_event(&mut self, mut until: impl FnMut(&mut Self) -> Option<Event>) -> Result<Option<Event>> {
        loop {
...
impl<Dpy: Display + ?Sized> SpecialEventDisplay<Dpy> {
    ...
    fn wait_for_some_event(&mut self, mut until: impl FnMut(&mut Self) -> Option<Event>) -> Result<Event> {
        loop {
            // poll for event from the inner display
...
impl<D: CanBeAsyncDisplay + ?Sized> CanBeAsyncDisplay for SpecialEventDisplay<D> {
    fn format_request(&mut self, req: &mut RawRequest, ctx: &mut Context< '_>) ->Result<AsyncStatus<u64>> {
    ...
    fn try_send_request_raw(&mut self, req: &mut RawRequest, ctx: &mut Context< '_>) ->Result<AsyncStatus<()>> {
